        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    diff = tf.tile(tf.abs(gpu_target_dst_masked_opt - gpu_pred_dst_dst_masked_opt), (5, 1, 1, 1, 1))
                    gpu_dst_loss += 1 * tf.reduce_mean ( tf.reduce_sum(gaussian_weights[-1, :, :, :, :] * diff, axis=[0, 3, 4]), axis=[2])
