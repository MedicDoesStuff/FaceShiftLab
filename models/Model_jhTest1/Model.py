...
""").lower()
    ...
    def on_initialize(self):
        ...
        model_archi = nn.JhTestArchi(resolution, opts=archi_opts)
    ...
    def onGetPreview(self, samples):
        ...
        if self.resolution <= 256:
            ...
            result += [ ('jhTest1', np.concatenate (st, axis=0 )), ]
            ...
            result += [ ('jhTest1 masked', np.concatenate (st_m, axis=0 )), ]
        else:
            result = []
            ...
            for i in range(n_samples):
                ar = S[i], SS[i]
                st.append ( np.concatenate ( ar, axis=1) )
            result += [ ('jhTest1 src-src', np.concatenate (st, axis=0 )), ]

            st = []
            for i in range(n_samples):
                ar = D[i], DD[i]
                st.append ( np.concatenate ( ar, axis=1) )
            result += [ ('jhTest1 dst-dst', np.concatenate (st, axis=0 )), ]

            st = []
            for i in range(n_samples):
                ar = D[i], SD[i]
                st.append ( np.concatenate ( ar, axis=1) )
            result += [ ('jhTest1 pred', np.concatenate (st, axis=0 )), ]
            ...
            for i in range(n_samples):
                ...
                st_m.append ( np.concatenate ( ar, axis=1) )
            result += [ ('jhTest1 masked src-src', np.concatenate (st_m, axis=0 )), ]

            st_m = []
            for i in range(n_samples):
                ar = D[i]*target_dstm[i], DD[i]*DDM[i]
                st_m.append ( np.concatenate ( ar, axis=1) )
            result += [ ('jhTest1 masked dst-dst', np.concatenate (st_m, axis=0 )), ]
            ...
            for i in range(n_samples):
                ...
                st_m.append ( np.concatenate ( ar, axis=1) )
            result += [ ('jhTest1 masked pred', np.concatenate (st_m, axis=0 )), ]
