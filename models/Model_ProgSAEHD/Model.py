        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    gpu_src_loss += tf.reduce_mean ( gaussian_weights[-1, :, :, :, :] * tf.abs ( gpu_target_src_masked_opt - gpu_pred_src_src_masked_opt ), axis=[1, 2, 3])
                    ...
                    gpu_dst_loss += tf.reduce_mean ( gaussian_weights[-1, :, :, :, :] * tf.abs ( gpu_target_dst_masked_opt- gpu_pred_dst_dst_masked_opt ), axis=[1, 2, 3])
